...
void permutation(string ques, string ans)
{
    ...
    for (int i = 0; i < ques.length(); i++)
    {
        int mask = 1 << (ques[i] - 'a');
        // if (arr[ques[i] - 'a'] == 0)
        if ((arr & mask) == 0)  //this means ye abhi unset pada hai.....
        {
            // arr[ques[i] - 'a'] = 1;
            arr |= mask;  //to set this bit we use or operator....
...
int queenBoxCombi(int tnq, int tnb, int lqpsf, string ans)//tnq=total no of queens. //tnb=total no of boxes 
{
    if (tnq == 0)
    {
        cout << ans << endl;
